...
const responseFailedCodes = new Set(["agent_not_found", "agent_not_ready", "invalid_key", "agent_send_keys_failed"])
const agentStartBusyCode = "agent_pane_busy"
const agentStartAttemptBudget = 10
const agentStartRetryDelayMs = 100
...
const resumableRepairStateValues = [
	"repair_starting",
	"repair_timed_out",
	"repair_blocked",
	"repair_responded",
...
type CommandName = "run" | "respond" | "resume" | "review" | "decide" | "repair" | "cleanup" | "unknown"
...
const resumableRepairStates = new Set<RunState>(resumableRepairStateValues)
...
interface CurrentPane extends ControllerIdentity {
...
function privateTransitionPrompt(path: string, transition: "review" | "repair"): Uint8Array {
	...
	if (!existsSync(path) || !lstatSync(path).isFile()) {
		throw new FrontierError({
			...
			nextAction:
				transition === "review"
					? "Write the bounded review prompt to one private regular file."
					: "Write Nathan's exact approved repair prompt to one private regular file.",
		})
	}
...
function responseInput(path: string): ResponseInput {
	if (!existsSync(path) || !lstatSync(path).isFile()) {
		throw new FrontierError({
...
function runAgentStart(arguments_: string[], cwd: string): HerdrResult {
	// agent_pane_busy is Herdr's structured pre-launch rejection for a just-split pane:
	// nothing was spawned, so re-issuing the identical start is side-effect-free and the
	// unique agent name means a retry can never create a second worker.
	let result = runHerdr(arguments_, cwd)
	for (let attempt = 1; attempt < agentStartAttemptBudget; attempt += 1) {
		if (result.exitCode === 0 || result.errorCode !== agentStartBusyCode) return result
...
function agentStartRejectedBusy(result: HerdrResult): boolean {
...
function responseReviewerMatches(
	response: Record<string, unknown> | undefined,
	receipt: Receipt,
): response is Record<string, unknown> {
	const agent = jsonObject(response?.agent)
	return Boolean(
		receipt.resources.reviewerName &&
			...
			stringField(agent, "name") === receipt.resources.reviewerName &&
			stringField(agent, "pane_id") === receipt.resources.reviewerPaneId &&
			stringField(agent, "workspace_id") === receipt.workspace.workspaceId &&
			stringField(agent, "tab_id") === receipt.workspace.tabId,
	)
}
...
function controllerMatchesCheckpoint(
	controller: ControllerIdentity,
	checkpoint: ControllerIdentity | undefined,
): checkpoint is ControllerIdentity {
	return Boolean(
		checkpoint &&
			controller.sessionId === checkpoint.sessionId &&
			controller.workspaceId === checkpoint.workspaceId &&
			controller.tabId === checkpoint.tabId &&
			controller.paneId === checkpoint.paneId,
	)
}

function controllerIdentityValid(controller: ControllerIdentity, workspace: Receipt["workspace"]): boolean {
	return (
		controller.sessionId.length > 0 &&
		controller.workspaceId === workspace.workspaceId &&
		controller.tabId === workspace.tabId &&
		controller.paneId === workspace.callerPaneId
	)
}

function controllerIdentityRecordValid(controller: Record<string, unknown> | undefined): boolean {
	return Boolean(
...
function responseEffectValid(receipt: Receipt): boolean {
	const effect = receipt.effects.responseDispatch
	if (!receipt.response && !effect) return true
	return Boolean(
		receipt.response &&
			sha256Pattern.test(receipt.response.sha256) &&
			effect &&
			effect.outcome !== "planned",
	)
}

function completedObservationValid(receipt: Receipt): boolean {
...
function reviewRecordValid(receipt: Receipt): boolean {
	if (!receipt.review) return false
	if (
		!sha256Pattern.test(receipt.review.promptSha256) ||
		!sha256Pattern.test(receipt.review.candidateBeforeSha256) ||
		(receipt.review.candidateAfterSha256 !== undefined &&
			!sha256Pattern.test(receipt.review.candidateAfterSha256)) ||
		(receipt.review.verdictMarkerSha256 !== undefined &&
			!sha256Pattern.test(receipt.review.verdictMarkerSha256)) ||
		(receipt.review.verdict !== undefined &&
			!["approve", "request_changes", "reject", "cancelled"].includes(receipt.review.verdict))
	) {
		return false
	}
	return Boolean(
		receipt.resources.reviewerName &&
		completedObservationValid(receipt) &&
		receipt.effects.reviewerPane &&
		receipt.effects.reviewerStart &&
		receipt.effects.reviewPromptDispatch,
	)
}
	...
	return Boolean(
		["accepted", "declined"].includes(decision.classification) &&
			...
			decision.reviewerName === receipt.resources.reviewerName &&
				...
				controllerIdentityValid(decision.controller, receipt.workspace) &&
				decision.submittedAt.length > 0,
	)
}

function repairRecordValid(receipt: Receipt): boolean {
	const repair = receipt.repair
	const review = receipt.review
	if (!repair || !review) return false
	return Boolean(
		sha256Pattern.test(repair.promptSha256) &&
			repair.submissionAttempt === 1 &&
			repair.runId === receipt.runId &&
			sha256Pattern.test(repair.candidateBeforeSha256) &&
			(repair.candidateAfterSha256 === undefined || sha256Pattern.test(repair.candidateAfterSha256)) &&
			(repair.resultSha256 === undefined || sha256Pattern.test(repair.resultSha256)) &&
			(repair.resultMarkerSha256 === undefined || sha256Pattern.test(repair.resultMarkerSha256)) &&
			repair.candidateBeforeSha256 === review.candidateBeforeSha256 &&
			repair.candidateBeforeSha256 === review.candidateAfterSha256 &&
			repair.reviewerName === receipt.resources.reviewerName &&
			repair.verdict === "request_changes" &&
			review.verdict === "request_changes" &&
			repair.verdictMarkerSha256 === review.verdictMarkerSha256 &&
				repair.workerName === receipt.resources.workerName &&
				repair.workerPaneId === receipt.resources.workerPaneId &&
				controllerIdentityValid(repair.controller, receipt.workspace) &&
				repair.submittedAt.length > 0 &&
			(repair.completedAt === undefined || repair.completedAt.length > 0) &&
			(repair.responseSha256 === undefined || sha256Pattern.test(repair.responseSha256)) &&
			(repair.workerStatus === undefined || agentStatuses.has(repair.workerStatus)),
	)
}

function repairResponseEffectValid(receipt: Receipt): boolean {
	const responseSha256 = receipt.repair?.responseSha256
	const effect = receipt.effects.repairResponseDispatch
	if (!responseSha256 && !effect) return true
	return Boolean(responseSha256 && sha256Pattern.test(responseSha256) && effect && effect.outcome !== "planned")
}

function isResumableRepairState(state: RunState): boolean {
...
function receiptStateInvariants(receipt: Receipt): boolean {
	...
	if (receipt.repair && !repairRecordValid(receipt)) return false
	if (!repairResponseEffectValid(receipt)) return false

	if (receipt.state === "prepared") {
		return (
			!receiptHasRunResources(receipt) &&
			!receipt.response &&
			!receipt.effects.responseDispatch &&
			initialEffectNames.every((effect) => effectIs(receipt, effect, "planned"))
		)
	}
	if (receipt.state === "timed_out") {
		const promptTimedOut =
			completedRunEffects(receipt) &&
		...
		const responseTimedOut =
			completedRunEffects(receipt) &&
			effectIs(receipt, "responseDispatch", "timed_out") &&
			receipt.effects.responseDispatch?.code === "timeout"
		return promptTimedOut || responseTimedOut
	}
	if (receipt.state === "blocked") {
		return (
			completedRunEffects(receipt) &&
			receipt.observation?.agentStatus === "blocked" &&
			!effectIs(receipt, "responseDispatch", "succeeded")
		)
	}
	...
	if (
		...
		if (!reviewRecordValid(receipt)) return false
		if (receipt.state === "reviewed") {
			return Boolean(
				receipt.review?.verdict &&
				receipt.review.verdictMarkerSha256 &&
				receipt.review.candidateAfterSha256 === receipt.review.candidateBeforeSha256,
			)
		}
		if (receipt.state === "review_breached") {
			return Boolean(
				receipt.review?.candidateAfterSha256 &&
				receipt.review.candidateAfterSha256 !== receipt.review.candidateBeforeSha256 &&
				!receipt.review.verdict,
			)
		}
	...
	if (isResumableRepairState(receipt.state) || receipt.state === "repaired") {
		if (!repairRecordValid(receipt)) return false
		...
		if (receipt.state === "repair_timed_out") {
			return (
				effectIs(receipt, "repairPromptDispatch", "timed_out") ||
				effectIs(receipt, "repairWait", "timed_out") ||
				effectIs(receipt, "repairResponseDispatch", "timed_out", "unknown")
			)
		}
		if (receipt.state === "repair_blocked") {
			return Boolean(
				receipt.repair?.workerStatus === "blocked" &&
					(!receipt.repair.responseSha256 ||
						effectIs(receipt, "repairResponseDispatch", "unknown")),
			)
		}
		...
		if (receipt.state === "repair_unproved") {
			return Boolean(
				receipt.repair?.candidateAfterSha256 &&
					receipt.repair.resultSha256 &&
					!receipt.repair.resultMarkerSha256 &&
					!receipt.repair.completedAt,
			)
		}
		return Boolean(
			receipt.repair?.candidateAfterSha256 &&
				receipt.repair.candidateAfterSha256 !== receipt.repair.candidateBeforeSha256 &&
				receipt.repair.resultSha256 &&
				receipt.repair.resultSha256 !== receipt.observation?.resultSha256 &&
				receipt.repair.resultMarkerSha256 ===
					sha256(`frontier-result:${receipt.repair.resultSha256}`) &&
				receipt.repair.completedAt &&
				effectIs(receipt, "repairPromptDispatch", "succeeded") &&
				!receipt.decision,
		)
	}
...
function isReceipt(value: unknown): value is Receipt {
	...
	if (
		...
		(receipt?.decision !== undefined &&
			(!decision ||
				!controllerIdentityRecordValid(decisionControllerRecord) ||
		...
		(receipt?.repair !== undefined &&
			(!repair ||
				!controllerIdentityRecordValid(repairControllerRecord) ||
				!sha256Pattern.test(stringField(repair, "promptSha256") ?? "") ||
				repair.submissionAttempt !== 1 ||
				!stringField(repair, "runId") ||
				!sha256Pattern.test(stringField(repair, "candidateBeforeSha256") ?? "") ||
				!stringField(repair, "reviewerName") ||
				stringField(repair, "verdict") !== "request_changes" ||
				!sha256Pattern.test(stringField(repair, "verdictMarkerSha256") ?? "") ||
				!stringField(repair, "workerName") ||
				!stringField(repair, "workerPaneId") ||
				!stringField(repair, "submittedAt") ||
				(repair.candidateAfterSha256 !== undefined &&
					!sha256Pattern.test(stringField(repair, "candidateAfterSha256") ?? "")) ||
				(repair.resultSha256 !== undefined &&
					!sha256Pattern.test(stringField(repair, "resultSha256") ?? "")) ||
				(repair.resultMarkerSha256 !== undefined &&
					!sha256Pattern.test(stringField(repair, "resultMarkerSha256") ?? "")) ||
				(repair.completedAt !== undefined && !stringField(repair, "completedAt")) ||
				(repair.workerStatus !== undefined && !stringField(repair, "workerStatus")) ||
				(repair.responseSha256 !== undefined &&
					!sha256Pattern.test(stringField(repair, "responseSha256") ?? "")))) ||
		!effects
	) {
		return false
	}
...
function splitPane(
	receipt: Receipt,
	effect: EffectName,
	resource: "editorPaneId" | "browserPaneId" | "workerPaneId" | "reviewerPaneId",
	sourcePaneId: string,
	direction: "right" | "down",
): string {
	if (receipt.state !== "review_starting") receipt.state = "starting"
...
function waitForWorker(receipt: Receipt, phase: "run" | "repair" = "run"): AgentObservation {
	const repair = phase === "repair"
	const waitEffect: EffectName = repair ? "repairWait" : "resumeWait"
	const result = runHerdr(
	...
	if (result.exitCode !== 0) {
		if (result.errorCode === "timeout") {
			...
			throw new FrontierError({
				...
				retrySafe: true,
				nextAction: repair
					? `Run frontier-runner resume --run-id ${receipt.runId}; the repair prompt will not be resent.`
					: `Run frontier-runner resume --run-id ${receipt.runId} again later; no prompt will be resent.`,
			})
		}
		throw new FrontierError({
			...
			state: receipt.state,
			nextAction: repair
				? `Inspect ${receipt.resources.workerName}; do not replace it or resend the repair prompt.`
				: `Inspect ${receipt.resources.workerName} with herdr agent get and agent read.`,
		})
	}
...
function readSettledWorker(
	receipt: Receipt,
	observation: AgentObservation,
	phase: "run" | "repair",
): { resultSha256: string; marker: string; readback: string } {
	const repair = phase === "repair"
	if (!completedStatuses.has(observation.status)) {
		throw new FrontierError({
			...
			retrySafe: true,
			nextAction: repair
				? `Inspect ${receipt.resources.workerName}; resume later without resending the repair prompt.`
				: `Run frontier-runner resume --run-id ${receipt.runId} to wait without resending the prompt.`,
		})
	}
	...
	if (read.exitCode !== 0) {
		throw new FrontierError({
			...
			retrySafe: true,
			nextAction: repair
				? `Inspect ${receipt.resources.workerName}, then resume without resending the repair prompt.`
				: `Inspect ${receipt.resources.workerName} directly, then resume without resending the prompt.`,
		})
	}
...
function proveResult(receipt: Receipt, observation: AgentObservation): ResultEnvelope {
	if (observation.status === "blocked") {
		...
		throw new FrontierError({
			...
			nextAction: responseAttempted
				? `Inspect ${receipt.resources.workerName}; the recorded response will not be sent again.`
				: `Inspect ${receipt.resources.workerName}, obtain exact operator approval, then use frontier-runner respond.`,
		})
	}
...
function unknownEffect(receipt: Receipt, ignored: readonly EffectName[] = []): string | undefined {
	const ignoredSet = new Set<EffectName>(ignored)
	return Object.entries(receipt.effects).find(
		([name, effect]) => effect.outcome === "unknown" && !ignoredSet.has(name as EffectName),
	)?.[0]
}
...
function respondCommand(arguments_: string[]): ResultEnvelope {
	...
	if (repairResponse) requireCheckpointedRepairController(receipt)
	if (
		(repairResponse && (receipt.repair?.responseSha256 || receipt.effects.repairResponseDispatch)) ||
		(!repairResponse && (receipt.response || receipt.effects.responseDispatch))
	) {
		throw new FrontierError({
	...
	if (receipt.state === "completed" || receipt.state === "repaired" || receipt.state === "cleaned") {
		throw new FrontierError({
			...
			state: receipt.state,
			nextAction:
				receipt.state === "completed" || receipt.state === "repaired"
					? `Run frontier-runner cleanup --run-id ${runId} when the panes are no longer needed.`
					: "Use a new unit ID for another bounded run.",
		})
	}
	...
	const responseEffect: EffectName = repairResponse ? "repairResponseDispatch" : "responseDispatch"
	recordEffect(receipt, responseEffect, "unknown")
	const result = runHerdr(
		["agent", "send-keys", receipt.resources.workerName, ...response.keys, "enter"],
		receipt.workspace.path,
	)
...
function reviewerObservation(receipt: Receipt): AgentObservation {
...
function waitForReviewer(receipt: Receipt): AgentObservation {
	const result = runHerdr(
		["agent", "wait", receipt.resources.reviewerName!, "--timeout", String(receipt.inputs.timeoutMs)],
		receipt.workspace.path,
	)
	if (result.exitCode !== 0) {
		if (result.errorCode === "timeout") {
...
function reviewMarkers(readback: string): Array<{ marker: string; parsed: Record<string, unknown> }> {
	const prefix = "frontier-review:"
	const lines = readback.split(/\r?\n/)
	const markers: Array<{ marker: string; parsed: Record<string, unknown> }> = []
	for (let index = 0; index < lines.length; index += 1) {
		...
		for (let end = index; end < Math.min(lines.length, index + 24); end += 1) {
			if (end > index) jsonText += (lines[end] ?? "").trim()
			...
			if (
				keys === "candidateSha256,schemaVersion,verdict" &&
				parsed.schemaVersion === 1 &&
				sha256Pattern.test(stringField(parsed, "candidateSha256") ?? "") &&
				["approve", "request_changes", "reject", "cancelled"].includes(verdict ?? "")
			) {
...
function proveReview(receipt: Receipt, observation: AgentObservation): ResultEnvelope {
	if (observation.status === "working") observation = waitForReviewer(receipt)
	if (!completedStatuses.has(observation.status)) {
		throw new FrontierError({
	...
	const read = runHerdr(
		["agent", "read", receipt.resources.reviewerName!, "--source", "recent-unwrapped", "--lines", "120"],
		receipt.workspace.path,
	)
	if (read.exitCode !== 0) {
		throw new FrontierError({
	...
	receipt.review!.verdict = verdict as ReviewVerdict
...
function reviewCommand(arguments_: string[]): ResultEnvelope {
	...
	const createdAt = now()
	receipt.state = "review_starting"
	receipt.resources.reviewerName = `frr_${sha256(`${runId}\0review`).slice(0, 12)}`
	...
	writeReceipt(receipt)
	receipt.resources.reviewerPaneId = splitPane(
		receipt,
		"reviewerPane",
		"reviewerPaneId",
		receipt.workspace.callerPaneId,
...
function recordedController(receipt: Receipt, transition: "decision" | "repair"): CurrentPane {
...
function requireCheckpointedRepairController(receipt: Receipt): CurrentPane {
...
function requireReviewedCandidate(receipt: Receipt, transition: "decision" | "repair"): void {
	const resultFile = canonicalResultFile(receipt.workspace.path, receipt.inputs.resultFile)
	if (
...
function proveRepair(receipt: Receipt, observation: AgentObservation, command: "repair" | "resume"): ResultEnvelope {
	if (observation.status === "working") observation = waitForWorker(receipt, "repair")
	if (observation.status === "blocked") {
	...
	receipt.repair!.workerStatus = observation.status
	if (
		candidateAfterSha256 === receipt.repair!.candidateBeforeSha256 ||
		resultSha256 === receipt.observation?.resultSha256 ||
		exactMarkerCount(readback, marker) !== 1
	) {
...
function repairCommand(arguments_: string[]): ResultEnvelope {
...
function resumeCommand(arguments_: string[]): ResultEnvelope {
	...
	if (isResumableRepairState(receipt.state)) {
		...
		const uncertainEffect = unknownEffect(receipt, ["repairPromptDispatch", "repairResponseDispatch"])
		if (uncertainEffect) {
			throw new FrontierError({
				message: `receipt contains an unknown external effect: ${uncertainEffect}`,
				code: "EFFECT_UNKNOWN",
	...
	if (
		...
		const uncertainEffect = unknownEffect(receipt, ["reviewPromptDispatch"])
		if (uncertainEffect) {
			throw new FrontierError({
				message: `receipt contains an unknown external effect: ${uncertainEffect}`,
				code: "EFFECT_UNKNOWN",
...
function cleanupCommand(arguments_: string[]): ResultEnvelope {
	...
	for (const splitEffect of ["editorPane", "browserPane", "workerPane", "reviewerPane"] as const) {
...
try {
	const envelope =
		command === "run"
			...
			: command === "respond"
				? respondCommand(commandArguments)
				: command === "resume"
					...
					: command === "review"
						...
						: command === "decide"
							...
							: command === "repair"
								? repairCommand(commandArguments)
