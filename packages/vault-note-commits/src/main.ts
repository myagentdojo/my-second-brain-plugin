...
function flags(args: string[], allowed: Set<string>): Map<string, string[]> {
...
function stateRoot(): string {
	...
	if (!root) refuse("begin", "STATE_HOME_MISSING", null, "Set XDG_STATE_HOME or HOME, then retry begin.")
...
function candidateCommit(manifest: Manifest, message: string): string {
...
function ownerIsLive(path: string): boolean {
	try {
		...
		if (!Number.isInteger(owner.pid) || !owner.pid) return true
...
function integrate(manifest: Manifest, commit: string): Result {
	return withLock(manifest, () => {
		...
		return outcome(true, "finish", "INTEGRATED", manifest.runId, removed.exitCode === 0
			? "Run remote sync separately when you want to publish main."
			: "The commit is integrated; remove the preserved candidate worktree when convenient.", {
...
function main(): void {
	...
	try {
		...
		const result = error instanceof Refusal
			? error.result
			: outcome(false, "help", "UNEXPECTED_FAILURE", null, "Preserve any candidate worktree and inspect the local error before retrying.", { retrySafe: false })
