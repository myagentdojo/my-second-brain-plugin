...
// packages/agent-browser/src/modules/private-delivery/private-entry.ts
var compiledSkillEnvironmentName = "AGENT_PLUGIN_COMPILED_SKILL";
function currentCompiledSkill() {
  return process.env[compiledSkillEnvironmentName];
}
function privateEntryPath(input) {
  return input.compiledSkill === "agent-browser" ? input.executable : input.invocationEntry;
}
function privateEntryCommand(input) {
