...
3. `Available portable skills`: list `hello-world`, `skill-a`, `skill-b`, `capability-tour`, `dev-mode`, `frontier-runner`, `handoff-to-opus`, `new-note`, `new-project`, `runtime-custody`, and `ultragoal`, marking the first three Bun-backed and the last eight model-only.
